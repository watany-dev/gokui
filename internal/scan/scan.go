...
var (
	...
	curlPipeSourceStdInExecPattern  = regexp.MustCompile(`(?i)\b(?:curl|wget)\b[^\n|]{0,300}\|\s*(?:(?:builtin|command)(?:\s*--|\s+-p(?:\s*--)?|-p(?:\s*--)?)?\s+)*(?:source|\.)\s+(?:\\*['"])?(?:/+dev/+stdin|/+dev/+fd/+0+|/+proc/+(?:self|thread-self|\$\$|\$[a-z_][a-z0-9_]*|\$\{[a-z_][a-z0-9_]*\}|\$\{[a-z_][a-z0-9_]*:-[a-z0-9_]+\}|\$[0-9]{1,10}|\$\{[0-9]{1,10}\}|\$\{[0-9]{1,10}:-[a-z0-9_]+\}|[0-9]{1,10})(?:/+task/+(?:\$\$|\$[a-z_][a-z0-9_]*|\$\{[a-z_][a-z0-9_]*\}|\$\{[a-z_][a-z0-9_]*:-[a-z0-9_]+\}|\$[0-9]{1,10}|\$\{[0-9]{1,10}\}|\$\{[0-9]{1,10}:-[a-z0-9_]+\}|[0-9]{1,10}))?/+fd/+0+|-)(?:\\*['"])?(?:\s|$|[,&;|)"'\x60])`)
