...
var (
	...
	pythonRemoteExecPattern     = regexp.MustCompile(`(?i)\bexec\s*\(\s*(?:requests\.get\s*\(\s*['"]https?://[^'"]+['"]\s*\)\.text|urllib\.request\.urlopen\s*\(\s*['"]https?://[^'"]+['"]\s*\)\.read\s*\(\s*\))`)
	nodeRemoteEvalPattern       = regexp.MustCompile(`(?i)\beval\s*\(\s*(?:await\s+)?fetch\s*\(\s*['"]https?://[^'"]+['"]\s*\)`)
