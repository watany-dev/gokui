...
func TestReleaseCheckFailsClosedWhenCleanupRemovalFails(t *testing.T) {
	...
	if !strings.Contains(out, "[RC_CLEANUP_REMOVE_FAILED]") {
		t.Fatalf("expected cleanup failure code in output, got:\n%s", out)
	}
	...
	if !strings.Contains(out, "[RC_CLEANUP_REMOVE_FAILED_SUMMARY] release-check cleanup failed for 2 output path(s)") {
		t.Fatalf("expected cleanup failure summary with failure count, got:\n%s", out)
	}
