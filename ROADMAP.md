...
- URL risk classification for shortener hosts and raw-IP URLs (including scheme-relative `//host/...` forms, bracketed/zone-id IPv6 hosts, and normalized trailing-dot/IDNA dot-variant hosts)
...
- markdown link-spoofing detection (display host vs target host mismatch), including inline and reference-style links (including spaced and one-line-break-separated reference forms)
