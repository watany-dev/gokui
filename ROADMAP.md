...
- pipe-to-stdin source/dot detection covers quoted and escaped-quoted stdin targets (including `/dev/stdin`, `/dev/fd/0`, `/proc/self/fd/0`, `/proc/thread-self/fd/0`, `/proc/<pid>/fd/0`, `/proc/<pid>/task/<tid>/fd/0`, and `-`), optional or stacked `builtin`/`command` prefixes, delimiter-terminated forms, and chains embedded in quoted command strings
