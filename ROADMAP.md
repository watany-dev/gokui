...
- inspect-sarif output path hardening (repository-root-only and outside-`.git` output enforcement, non-directory file-path enforcement, `..` path-segment rejection, symlink path-component rejection, restrictive SARIF file permissions, fail-closed output-collision checks, and atomic file creation with descriptor-backed writes)
