...
- URL risk classification for paste-site URLs and GitHub release asset URLs (including known GitHub CDN hosts, API release-asset forms on `api.github.com`/`uploads.github.com` such as `releases/<release_id>/assets`, object-path forms, and `releases/latest/download` paths)
