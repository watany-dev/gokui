...
- pipe-to-stdin source/dot detection covers quoted and escaped-quoted stdin targets (including `/dev/stdin`, `/dev/fd/0`, `/proc/self/fd/0`, `/proc/thread-self/fd/0`, `/proc/<pid>/fd/0`, `/proc/<pid>/task/<tid>/fd/0`, and `-`), optional or stacked `builtin`/`command` prefixes (including `command --` / `builtin --` forms and attached `command--` / `builtin--` forms), equivalent stdin path spellings (for example `/dev//stdin` and `fd/00...` forms), delimiter-terminated forms, and chains embedded in quoted command strings
