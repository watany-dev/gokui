...
- `base64 -d | sh`, `xxd -r -p | sh`, decode output piped to `source`/`.` via stdin (including `/dev/fd/0`, `/proc/self/fd/0`, `/proc/thread-self/fd/0`, `/proc/<pid>/fd/0`, and `-` targets), `eval`, `EncodedCommand`, and similar obfuscation
