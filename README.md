...
| `RC_PREFLIGHT_SARIF_OUT_INVALID` | `RELEASE_CHECK_SARIF_OUT` is root-like (`/`, `.`, empty), ends with `/`, does not end with `.sarif`, contains `.`/`..` path segments, resolves outside the repository root, or resolves under `.git/` |
