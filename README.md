...
- `base64 -d | sh`, `xxd -r -p | sh`, decode output piped to `source`/`.` via stdin (including `/dev/fd/0`, `/proc/self/fd/0`, `/proc/thread-self/fd/0`, `/proc/<pid>/fd/0`, `/proc/<pid>/task/<tid>/fd/0`, and `-` targets), with optional or stacked `builtin`/`command` prefixes (including `command --` / `builtin --` forms) and equivalent stdin path spellings (for example `/dev//stdin` and `fd/00...` forms), `eval`, `EncodedCommand`, and similar obfuscation
